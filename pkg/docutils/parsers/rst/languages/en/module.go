...
func init() {
	Code = πg.NewCode("<module>", "/usr/lib/python2.7/site-packages/docutils/parsers/rst/languages/en.py", nil, 0, func(πF *πg.Frame, _ []*πg.Object) (*πg.Object, *πg.BaseException) {
		var πR *πg.Object
		_ = πR
		...
		ßsourcecode := πg.InternStr("sourcecode")
		...
		for ; πF.State() >= 0; πF.PopCheckpoint() {
			...
			πF.SetLineno(18)
			πTemp001 = πg.NewDict()
			if πE = πTemp001.SetItem(πF, ßattention.ToObject(), ßattention.ToObject()); πE != nil {
				continue
			}
			if πE = πTemp001.SetItem(πF, ßcaution.ToObject(), ßcaution.ToObject()); πE != nil {
				continue
			}
			if πE = πTemp001.SetItem(πF, ßcode.ToObject(), ßcode.ToObject()); πE != nil {
				continue
			}
			if πE = πTemp001.SetItem(πF, πg.NewStr("code-block").ToObject(), ßcode.ToObject()); πE != nil {
				continue
			}
			if πE = πTemp001.SetItem(πF, ßsourcecode.ToObject(), ßcode.ToObject()); πE != nil {
				continue
			}
			if πE = πTemp001.SetItem(πF, ßdanger.ToObject(), ßdanger.ToObject()); πE != nil {
				continue
			}
			...
			if πE = πTemp001.SetItem(πF, ßmath.ToObject(), ßmath.ToObject()); πE != nil {
				continue
			}
			if πE = πTemp001.SetItem(πF, ßimage.ToObject(), ßimage.ToObject()); πE != nil {
				continue
			}
			...
			// line 72: """English name to registered (in directives/__init__.py) directive name
			πF.SetLineno(72)
			// line 75: roles = {
			πF.SetLineno(75)
			πTemp001 = πg.NewDict()
			...
			if πE = πTemp001.SetItem(πF, ßsubscript.ToObject(), ßsubscript.ToObject()); πE != nil {
				continue
			}
			if πE = πTemp001.SetItem(πF, ßsub.ToObject(), ßsubscript.ToObject()); πE != nil {
				continue
			}
			if πE = πTemp001.SetItem(πF, ßsuperscript.ToObject(), ßsuperscript.ToObject()); πE != nil {
				continue
			}
			...
			// line 109: """Mapping of English role names to canonical role names for interpreted text.
			πF.SetLineno(109)
		}
		return nil, πE
	})
