...
func init() {
	Code = πg.NewCode("<module>", "/usr/lib/python2.7/site-packages/docutils-0.16.data/scripts/rst2latex.py", nil, 0, func(πF *πg.Frame, _ []*πg.Object) (*πg.Object, *πg.BaseException) {
		var πR *πg.Object
		_ = πR
		...
		Label3:
			...
			goto Label1
		Label1:
			// line 17: from docutils.core import publish_cmdline
			πF.SetLineno(17)
			...
			// line 19: description = ('Generates LaTeX documents from standalone reStructuredText '
			πF.SetLineno(19)
			if πE = πF.Globals().SetItem(πF, ßdescription.ToObject(), πg.NewStr("Generates LaTeX documents from standalone reStructuredText sources. Reads from <source> (default is stdin) and writes to <destination> (default is stdout).  See <http://docutils.sourceforge.net/docs/user/latex.html> for the full reference.").ToObject()); πE != nil {
				continue
			}
			// line 26: publish_cmdline(writer_name='latex', description=description)
			πF.SetLineno(26)
			if πTemp001, πE = πg.ResolveGlobal(πF, ßdescription); πE != nil {
				continue
			}
			πTemp006 = πg.KWArgs{
				{"writer_name", ßlatex.ToObject()},
