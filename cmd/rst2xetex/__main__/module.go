...
func init() {
	Code = πg.NewCode("<module>", "/usr/lib/python2.7/site-packages/docutils-0.16.data/scripts/rst2xetex.py", nil, 0, func(πF *πg.Frame, _ []*πg.Object) (*πg.Object, *πg.BaseException) {
		var πR *πg.Object
		_ = πR
		...
		Label1:
			...
			πF.SetLineno(19)
			if πE = πF.Globals().SetItem(πF, ßdescription.ToObject(), πg.NewStr("Generates LaTeX documents from standalone reStructuredText sources for compilation with the Unicode-aware TeX variants XeLaTeX or LuaLaTeX. Reads from <source> (default is stdin) and writes to <destination> (default is stdout).  See <http://docutils.sourceforge.net/docs/user/latex.html> for the full reference.").ToObject()); πE != nil {
				continue
			}
			// line 27: publish_cmdline(writer_name='xetex', description=description)
			πF.SetLineno(27)
			if πTemp001, πE = πg.ResolveGlobal(πF, ßdescription); πE != nil {
				continue
			}
			πTemp006 = πg.KWArgs{
				{"writer_name", ßxetex.ToObject()},
