...
func init() {
	Code = πg.NewCode("<module>", "/usr/lib/python2.7/site-packages/docutils-0.16.data/scripts/rst2html4.py", nil, 0, func(πF *πg.Frame, _ []*πg.Object) (*πg.Object, *πg.BaseException) {
		var πR *πg.Object
		_ = πR
		...
		for ; πF.State() >= 0; πF.PopCheckpoint() {
			...
			// line 16: locale.setlocale(locale.LC_ALL, '')
			πF.SetLineno(16)
		...
		Label3:
			...
			goto Label1
		Label1:
			// line 20: from docutils.core import publish_cmdline, default_description
			πF.SetLineno(20)
			...
			// line 23: description = ('Generates (X)HTML documents from standalone reStructuredText '
			πF.SetLineno(23)
			if πTemp003, πE = πg.ResolveGlobal(πF, ßdefault_description); πE != nil {
				continue
			}
			if πTemp001, πE = πg.Add(πF, πg.NewStr("Generates (X)HTML documents from standalone reStructuredText sources.  ").ToObject(), πTemp003); πE != nil {
				continue
			}
			if πE = πF.Globals().SetItem(πF, ßdescription.ToObject(), πTemp001); πE != nil {
				continue
			}
			// line 26: publish_cmdline(writer_name='html4', description=description)
			πF.SetLineno(26)
			if πTemp001, πE = πg.ResolveGlobal(πF, ßdescription); πE != nil {
				continue
			}
			πTemp006 = πg.KWArgs{
				{"writer_name", ßhtml4.ToObject()},
